/**
 * @file Defines the `/search-feat` slash command.
 * This command allows users to search for a D&D feat by name. If multiple
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('search-feat')
        .setDescription('Searches for a feat by name.')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const searchResponse = await axios.post(`${API_BASE_URL}/feat`, { query });
            const results = searchResponse.data.results;

            if (!results || results.length === 0) {
                await interaction.editReply({ content: `I couldn't find any feats matching "${query}".` });
                return;
            }
            ...
            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId(`select-feat:${sessionId}`)
                .setPlaceholder('Select a feat to view details')
                .addOptions(options.slice(0, 25));

            const row = new ActionRowBuilder().addComponents(selectMenu);

            const reply = await interaction.editReply({
                content: `I found ${results.length} feats matching "${query}". Please select one:`,
                components: [row],
            });

            const collector = reply.createMessageComponentCollector({
                filter: i => i.customId === `select-feat:${sessionId}` && i.user.id === interaction.user.id,
                time: 60000,
            });
            ...
            collector.on('collect', async i => {
                try {
                    ...
                    const selectedFeat = results[selectedIndex];
                    ...
                    const embed = formatDetailResult({ ...detailResponse.data, category: 'feats' });
