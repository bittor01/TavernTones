/**
 * @file Defines the `/search-deep` slash command.
 * This command performs a comprehensive search across all 5e content categories,
 * matching against both names and full text content. It presents results in a
 * dropdown and handles long responses by creating a thread.
 * @author jules
 */
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('search-deep')
        .setDescription('Searches all 5e content by name and content.')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const searchResponse = await axios.post(`${API_BASE_URL}/deep`, { query });
            ...
            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId(`select-deep:${sessionId}`)
            ...
            const collector = reply.createMessageComponentCollector({
                filter: i => i.customId === `select-deep:${sessionId}` && i.user.id === interaction.user.id,
                time: 60000,
            });
            ...
            collector.on('collect', async i => {
                try {
                    ...
                    // If the description is too long, create a thread for the full content.
                    if (description.length > 4000) {
                        const originalDescription = embed.data.description;
                        embed.setDescription('This response is too long to display in a single message. See the thread below for the full details.');
