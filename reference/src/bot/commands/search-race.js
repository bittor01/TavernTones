/**
 * @file Defines the `/search-race` slash command.
 * This command allows users to search for a D&D race or lineage by name. If multiple
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('search-race')
        .setDescription('Searches for a race by name.')
        .addStringOption(option =>
            option.setName('query')
                .setDescription('The name of the race to search for.')
                .setRequired(true)),
    /**
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const searchResponse = await axios.post(`${API_BASE_URL}/race`, { query });
            const results = searchResponse.data.results;

            if (!results || results.length === 0) {
                await interaction.editReply({ content: `I couldn't find any races matching "${query}".` });
                return;
            }
            ...
            const selectMenu = new StringSelectMenuBuilder()
                .setCustomId(`select-race:${sessionId}`)
                .setPlaceholder('Select a race to view details')
                .addOptions(options.slice(0, 25));

            const row = new ActionRowBuilder().addComponents(selectMenu);

            const reply = await interaction.editReply({
                content: `I found ${results.length} races matching "${query}". Please select one:`,
                components: [row],
            });

            const collector = reply.createMessageComponentCollector({
                filter: i => i.customId === `select-race:${sessionId}` && i.user.id === interaction.user.id,
                time: 60000,
            });
            ...
            collector.on('collect', async i => {
                try {
                    ...
                    const selectedRace = results[selectedIndex];
                    ...
                    const embed = formatDetailResult({ ...detailResponse.data, category: 'races' });
