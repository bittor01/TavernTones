...
async function apploader() {
    ...
    // Stage 1: Create window and load base IPC handlers
    await createWindow(false); // Create hidden
    isAppReady = true;
    loadBaseIpcHandlers();

    // Stage 2: Check for data path configuration
    ...
    if (!pathsConfigured) {
        logToRenderer("Essential data folders are not configured. Please set them in the Settings window.");
        ...
        return; // Halt further initialization until configured
    }

    // Stage 3: Paths are configured, load data-dependent modules and show window
    logToRenderer("Data paths found. Initializing core components.");
    ...
    // Stage 4: Initialize Discord bot if token is present
...
// STAGE 1: Load handlers that are always needed, especially for the settings window.
function loadBaseIpcHandlers() {
    logToRenderer("Loading Base IPC Handlers...");
    // Helper for folder selection dialogs
    ...
    ipcMain.handle('setup-default-folders', async () => {
        ...
        try {
            ...
            await dialog.showMessageBox(mainWindow, { type: 'info', title: 'Success', message: `Default folders created in:\n${dataDir}`, buttons: ['OK'] });
            return paths;
        } catch (error) {
    ...
    ipcMain.on('get-discord-config', async (event) => event.sender.send('discord-config', await getDiscordConfig()));
    ipcMain.on('set-discord-config', async (event, config) => {
        await setDiscordConfig(config);
        await dialog.showMessageBox(null, { type: 'info', title: 'Settings Saved', message: 'Your settings have been saved. Please restart the application to apply the changes.', buttons: ['OK'] });
        app.relaunch();
        app.quit();
    });
    ...
    ipcMain.handle('get-dnd-conditions', async () => DND_CONDITIONS);
...
// STAGE 2: Load handlers that depend on the data paths being configured.
function loadDataDependentIpcHandlers() {
    ...
    ipcMain.on('add-creature', (event, creature) => {
        ...
        if (rollLogMessage) mainWindow.webContents.send('dice-log', rollLogMessage);
    });
    ipcMain.on('update-initiative', (event, { creatureId, initiative }) => initiativeTracker.updateInitiative(creatureId, initiative));
    ...
    ipcMain.on('remove-creature', (event, { creatureId }) => initiativeTracker.removeCreature(creatureId));
    ...
    // ... add all other initiativeTracker handlers here ...
    ipcMain.on('add-temp-hp', (event, { creatureId, amount }) => initiativeTracker.addTempHp(creatureId, amount));
    ipcMain.on('add-condition', (event, { creatureId, condition }) => initiativeTracker.addCondition(creatureId, condition));
    ipcMain.on('remove-condition', (event, { creatureId, condition }) => initiativeTracker.removeCondition(creatureId, condition));
    ipcMain.on('update-creature-flag', (event, { creatureId, flag, value }) => initiativeTracker.updateCreatureFlag(creatureId, flag, value));
    ipcMain.on('edit-creature', (event, { creatureId }) => {
        const creature = initiativeTracker.editCreature(creatureId);
        if (creature) mainWindow.webContents.send('populate-edit-form', creature);
    });
    ipcMain.on('copy-creature', (event, { creatureId }) => {
        const creature = initiativeTracker.getCreature(creatureId);
        if (creature) mainWindow.webContents.send('populate-edit-form', creature);
    });
    ipcMain.on('update-reminders', (event, { creatureId, reminders }) => initiativeTracker.updateReminders(creatureId, reminders));
    ipcMain.on('roll-stat', (event, { creatureId, rollType, stat, type }) => {
        ...
        if (message) mainWindow.webContents.send('dice-log', message);
    });
    ipcMain.on('roll-attack', (event, { creatureId, rollType }) => {
        const message = initiativeTracker.rollAttack(creatureId, rollType);
        if (message) mainWindow.webContents.send('dice-log', message);
    });
    ipcMain.on('reset-encounter', () => initiativeTracker.resetEncounter());
    ipcMain.on('clear-encounter', async () => {
        ...
        if (result.response === 0) initiativeTracker.clearEncounter();
    });
    ipcMain.on('save-encounter', async () => {
        const { canceled, filePath } = await dialog.showSaveDialog(mainWindow, {
            title: 'Save Encounter',
            defaultPath: path.join(app.getPath('userData'), 'encounters'), // Suggest a subfolder
            filters: [{ name: 'JSON Files', extensions: ['json'] }]
        });
        if (!canceled && filePath) initiativeTracker.saveEncounterToFile(filePath);
    });
    ...
    // 5eParser Handlers
    ipcMain.handle('search-monsters', async (event, query) => fiveEToolsParser.searchByName('bestiary', query));
    ipcMain.handle('get-monster-details', async (event, { name, source }) => fiveEToolsParser.getExact('bestiary', name, source));

    // Gamify Tool Handlers
    ipcMain.handle('open-task-file-dialog', async () => {
        const { filePaths } = await dialog.showOpenDialog(gamifyWindow, {
            title: 'Select Task File',
            defaultPath: discordConfig.tasksPath,
            properties: ['openFile'],
            filters: [{ name: 'JSON Files', extensions: ['json'] }]
        });
        return filePaths && filePaths.length > 0 ? filePaths[0] : null;
    });
    // ... (rest of gamify handlers)
}

// STAGE 3: Load handlers that depend on the bot being connected.
function loadBotDependentIpcHandlers(client) {
    logToRenderer("Loading Bot-Dependent IPC Handlers...");
    // Music Player Handlers
    ipcMain.on('play-music', (event, filePath) => musicPlayer.playFile(filePath));
    ipcMain.on('pause-music', () => musicPlayer.pause());
...
client.once('clientReady', async () => {
    logToRenderer(`TavernTones is online as ${client.user.tag}!`);
    mainWindow.webContents.send('discord-bot-status', { status: 'online', message: `Logged in as ${client.user.tag}` });

    // --- Bot-Dependent Initializations ---
    ...
    loadBotDependentIpcHandlers(client);
    ...
    if (voiceChannel && voiceChannel.isVoiceBased()) {
        try {
            ...
            connection.on(VoiceConnectionStatus.Ready, () => {
                logToRenderer('The bot has connected to the voice channel!');
                musicPlayer.setConnection(connection);
            });
            // Add other connection status listeners...
