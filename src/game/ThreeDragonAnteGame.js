...
const { renderHand, renderDraftGrid } = require('./TDAImageGenerator.js');
...
class ThreeDragonAnteGame {
    ...
    async handleCommand(interaction) {
        if (this.activeGames.has(interaction.channel.id)) {
        ...
        game.players.push({ id: interaction.user.id, user: interaction.user, specialAbility: null, hand: [], flight: [], hoard: 0, dmMessages: { draftPage: 0, handPage: 0 }, draftedCards: [], isReady: false });
        this.activeGames.set(interaction.channel.id, game);

        const embed = this._generateLobbyEmbed(game);
        const components = this._buildLobbyComponents(game);

        // Send the main lobby message to the channel
        const lobbyMessage = await interaction.channel.send({ embeds: [embed], components });
        game.lobbyMessageId = lobbyMessage.id;

        // Acknowledge the command privately
        await interaction.reply({ content: 'Three-Dragon Ante lobby created!', ephemeral: true });
    ...
    _dealCard(game) {
        if (game.deck.length === 0) {
            if (game.discardPile.length === 0) {
                game.log.push('**Warning:** The deck and discard pile are both empty. No cards to draw.');
                return null;
            }
            game.log.push('The deck is empty. Shuffling the discard pile to create a new deck.');
    ...
    _createOpponentEmbed(opponent) {
        ...
        const flightValue = opponent.flight.length > 0
            ? opponent.flight.map(c => c.triggered ? this._formatCardName(c) : `~~${this._formatCardName(c)}~~`).join('\n')
            : 'None';
    ...
    async startDraftPhase(game) {
        ...
        game.draftGridImageBuffer = await renderDraftGrid({ cards: game.fullDraftPool });
    ...
    async handleDraftRemoval(interaction, game, cardName) {
        ...
        game.draftGridImageBuffer = await renderDraftGrid({
    ...
    async _requestCardPlay(game) {
        if (game.turnOrder.length === 0) {
            ...
            if (validPlays.length > 0) {
                ...
                game.log.push(`**${nextRoundLeader.user.username}** played the strongest unique card and is the leader for the next round.`);
            } else {
                game.log.push(`All cards played were tied. The leader remains **${nextRoundLeader.user.username}**.`);
            }
    ...
    async handleCardPlay(interaction, game, cardName) {
        ...
        if (isRoundLeader || (previousPlayerCard && card.value <= previousPlayerCard.value)) {
            ...
            game.log.push(`The power of **${this._formatCardName(card)}** did not trigger because it was stronger than the previous card played this round.`);
        }

        player.flight.push(card);
        game.lastRoundPlayed.push({ player, card });
        game.turnOrder.shift();
